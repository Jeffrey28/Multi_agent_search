...
This paper presents a measurement dissemination-based distributed Bayesian filtering (DBF) approach for a multi-robot network.
The DBF utilizes the Latest-In-and-Full-Out (LIFO) local exchange protocol of observations for data communication within the network.
% for target search and tracking.
Different from statistics dissemination-based approaches that transmit posterior distributions or likelihood functions, each robot under LIFO only exchanges with neighboring robots a full communication buffer consisting of latest available measurements,
%receives the latest available measurements and then broadcasts full communication buffer to its neighborhood, 
which significantly reduces the transmission burden between each pair of robots to scale linearly with the size of the network.
...
The design of distributed filtering algorithms depends on the communication topology of multi-robot network, which can be classified into two types: fusion center (FC)-based and neighborhood (NB)-based.
...
\todonote{write the observation model. refer to the paper ``Target Tracking by Particle Filtering in Binary Sensor Networks"}.
...
Each robot has its individual estimation of probability density function (PDF) of target position, called \textit{individual PDF}. 
...
In each step, LIFO only allows single-hopping communication within the direct neighborhood, but is able to broadcast observations of each robot to any other agent after a finite number of steps.
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		The $i^\text{th}$ robot receives all CBs of its direct neighborhood $\mathcal{N}_i$, each of which corresponds to the (k-1)-step CB of a robot in $\mathcal{N}_i$. 
...
\todonote{describe the connectivity with $M^k(i;j) > 0$ and probably generalize to directed topology.}
...
\todonote{these two corollaries seem straightforward to reviewers.}
...
\todonote{check whether the space complexity is correct}
\begin{rem}
...
Only proofs for localizing a static target are presented, including static robots and moving robots.
...
\todonote{the lack of proof for moving target case may be problematic. Maybe need to briefly describe how this can be done.}
...
Here, the main idea is to classify robot observation positions into two disjoint sets: \textit{infinite-observation spots} that contain positions where a robot makes infinite observations as time tends to infinity, and \textit{finite-observation spots} that contain positions where the robot makes finite observations.
...
%	\end{subfigure}
	\caption{(a)-(d) The $1^\text{st}$ robot's individual PDF at different times. The square denotes the current robot and stars represent other robots. The cross stands for the target. (e) Average position estimation errors of $1^\text{st}$, $3^\text{rd}$ and $5^\text{th}$ robot's LIFO-DBF, CbDF and CF. (f) Average entropy of individual PDFs.}
...
%	\end{subfigure}
	\caption{(a)-(d) The $6^\text{th}$ robot's individual PDFs at different times. (e) Average position estimation errors. (f) Average entropy of individual PDFs.}
...
%In all scenarios, each robot is equipped with a binary sensor. 
The networked robots take a ring communication topology that each robot can communicate with two fixed nearest neighbors.
...
Each robot follows a pre-defined circular trajectory.
%The third scenario contains six moving sensors and one moving target. 
A single-integrator kinematic model is used as the target motion model: \small$x^T(k+1)=x^T(k)+v\Delta T$\normalsize,
...
The CbDF requires robots to continually exchange their individual PDFs with direct neighbors, computing the average of all received and its own target PDFs. 
Multiple rounds of communication and averaging are conducted at each time step to ensure the convergence of each robot's individual PDFs.
...
\cref{fig:sta_sen_sta_tar} shows a test trial when both the robots and the target are static. %of the positions of six static robots are shown as stars and square.
Due to space constraint, only the individual PDF of the first robot is shown.
...
As more observations are fused, the individual PDF concentrates to the true location of the target (\cref{fig:sta_sen_sta_tar_sing4}), which accords with the consistency of LIFO-DBF.
...
This happens because the central unit has access to the latest observations of all robots, thus making most use of all available information.
It is worth noting that, LIFO-DBF achieves similar asymptotic performance as the CF, both in position estimation error and entropy reduction; this is achieved even though each robot only communicates with its two neighboring robots, which requires less communication burden than the CF.
...
\cref{fig:mov_sen_mov_tar} illustrates a test trial when the robots move in circles.
...
It can be noticed that the individual PDF concentrates to the true target location even when the target constantly moves.
...
This is not an unexpected result: due to the consensus procedure, CbDF is able to fuse the latest individual PDFs of each robot via the averaging process. 
This is especially useful when the environment is dynamically changing.
In contrast, LIFO-DBF for each robot only uses delayed information from neighboring robots, thus gaining less knowledge about the target's current position.
...
This paper presents a measurement dissemination-based distributed Bayesian filtering (DBF) approach for a multi-robot network, utilizing the Latest-In-and-Full-Out (LIFO) protocol for observation exchange.
...
By exchanging full communication buffers among neighboring robots, LIFO significantly reduces the transmission burden between each pair of robots to scale linearly with the network size.
%, with each robot non-intermittently receiving observations of all others.
...
Future work includes considering other types of sensors and imperfect communication between robots.
