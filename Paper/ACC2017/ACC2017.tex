...
Different from statistics dissemination-based approaches that transmit posterior distributions or likelihood functions, each UGV under LIFO only exchanges with neighboring UGVs a full communication buffer consisting of latest available measurements,
...
Each UGV is equipped with a binary sensor for environmental perception. The aim of UGVs is to efficiently localize a target in $S$. 
...
The target motion takes a discrete-time linear model that can described by
\begin{equation}
\label{eqn:tar_motion_model}
x^g_{k+1}=A_kx^g_k+B_ku_k,
\end{equation}
where $x^g_k=\begin{bmatrix}
x^g_k(1)\\x^g_k(2)
\end{bmatrix}$ denotes the target position at time $k$; $u_k\in\mathbb{R}^{2\times 1}$ represents the control input of the target; $A_k\in\mathbb{R}^{2\times 2},\;B_k\in\mathbb{R}^{2\times 2}$.

The binary sensor constantly measures the target state and 
The measurement model of $i^\text{th}$ UGV's sensor can be modeled as
\begin{equation*}
y^i_k=h^i(x^g_k),\; x^g_k\in S,
\end{equation*}
where $h^i$ is a sensor property that characterize the target state. For example, $h^i$ can be the power received by an ultrasonic sensor.
A binary sensor does not provide $y^i_k$ as its output, instead, it only gives one of two possible values: when the signal $y^i_k$ is greater than a threshold $\gamma$, the sensor gives $1$, indicating the target is detected in the sensor field of view; otherwise, $0$ is given by the sensor. Such model is shown below:
\begin{equation*}
z^i_k = 
\begin{cases}
...
\end{cases}
\end{equation*}
where $z^i_k$ denotes the observation of $i^\text{th}$ sensor at time $k$ and its value follows a Bernoulli distribution.
...
We can use a likelihood function to represent the probability of the target being detected by the binary sensor as a function of the target and sensor positions:
...
P(z^i_k=1|x^g_k;x^{R,i}_k)\in \left[0,1\right],\; x^g_k\in S
\end{equation}\normalsize
where $x^{R,i}_k$ is the $i^\text{th}$ sensor's position.
...
P(z^i_k=0|x^g_k;x^{R,i}_k)=1-P(z^i_k=1|x^g_k;x^{R,i}_k)
\end{equation}\normalsize
%\Cref{eqn:bin_sensor} actually defines a binary sensor model parameterized by $x^g_k$ and $x^R_k$.
The combination of \Cref{eqn:bin_sensor1} and \Cref{eqn:bin_sensor0} forms the probabilistic model for a binary sensor.
% parameterized by $x^g_k$ and $x^{R,i}_k$. 
...
The commonly used likelihood functions for binary sensors include Gaussian function \cite{bonnie2012modelling,liu2015model} and step function \cite{djuric2008target}.
...
All the UGVs in $\mathcal{N}_i$ can directly exchange information with $i^\text{th}$ UGV. 
...
\end{equation}\normalsize
where $P(x^g_k|x^g_{k-1})$ is a Markov motion model of the target, independent of UGV states. 
This model describes the state transition probability of the target from a prior state $x^g_{k-1}$ to posterior state $x^g_k$. 
...
%	(except current step CB) 
	...
	Therefore, LIFO-DBF sacrifices storage size for reducing communication burden. This is desirable for contemporary usage as local memory of vehicles are abundant compared to the limited bandwidth for communication.
\end{rem}

\section{Proof of Consistency}\label{sec:consist_proof} %and Consensus
This section proves the consistency of LIFO-DBF. 
...
%Considering $S$ is finite and $x^{T^*}$ is the true location of target, define an \textit{equivalent-location} set $x^g_{eq}\subseteq S$ such that 
%\small\begin{equation*}
%x^g_{eq}=\left\lbrace x^g\in S\arrowvert P(z_k|x^{T})=P(z_k|x^{T^*}),\; \forall z_k\in \left\lbrace 0,1\right\rbrace\right\rbrace ,
%\end{equation*}\normalsize
%i.e., $x^g\in x^g_{eq}$ gives the same observation likelihood as $x^{T^*}$ for given UGV positions.
...
%%\end{enumerate}
%%Without loss of generality, assume $x^{T^*}\in x^g_{eq,1}$, where $x^{T^*}$ denotes the actual position of the target. 
%
...
Comparing $P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})$ with $P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})$ yields
\small\begin{equation}\label{eqn:cmp}
\frac{P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})}=\frac{P^i_{pdf}(x^g)\prod\limits_{j=1}^{N}\prod\limits_{l=1}^{k^i_j}P(z^j_l|x^g)}{P^i_{pdf}(x^{T^*})\prod\limits_{j=1}^{N}\prod\limits_{l=1}^{k^i_j}P(z^j_l|x^{T^*})}
\end{equation}\normalsize

Take the logarithm of \Cref{eqn:cmp} and average it over $k$ steps:
\small\begin{equation}\label{eqn:cmp_log}
 \frac{1}{k}\ln\frac{P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})}=\frac{1}{k}\ln\frac{P^i_{pdf}(x^g)}{P^i_{pdf}(x^{T^*})}+\sum\limits_{j=1}^{N}\frac{1}{k}\sum\limits_{l=1}^{k^i_j}\ln\frac{P(z^j_l|x^g)}{P(z^j_l|x^{T^*})}.
\end{equation}\normalsize

Since $P^i_{pdf}(x^g)$ and $P^i_{pdf}(x^{T^*})$ are bounded, then
\small\begin{equation}\label{eqn:cmp_lim1}
\lim\limits_{k\rightarrow \infty}\frac{1}{k}\ln\frac{P^i_{pdf}(x^g)}{P^i_{pdf}(x^{T^*})}= 0.
\end{equation}\normalsize
...
%\lim\limits_{k\rightarrow \infty}
\frac{1}{k}\sum\limits_{l=1}^{k_j}\ln\frac{P(z^j_l|x^g)}{P(z^j_l|x^{T^*})}\overset{P}{\longrightarrow}p^*_j\ln \frac{p_j}{p^*_j}+(1-p^*_j)\ln \frac{1-p_j}{1-p^*_j}
\end{equation}\normalsize
...
% \lim\limits_{k\rightarrow \infty}
 \frac{1}{k}\ln\frac{P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})}\overset{P}{\longrightarrow}c(x^g)
...
\frac{P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})e^{c(x^g)k}}\overset{P}{\longrightarrow}1
\end{equation}\normalsize

Define the set $\bar{X}^T=S\setminus \left\lbrace x^{T^*}\right\rbrace$ and $c_M=\max\limits_{x^g\in\bar{X}^T}c(x^g)$.
Then $c_M<0$.
Summing \Cref{eqn:lim2} over $\bar{X}^T$ yields
\small\begin{equation}\label{eqn:lim3}
\frac{\sum\limits_{x^g\in \bar{X}^T}P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})e^{\left[c_M-c(x^g)\right]k}}{{P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})}e^{c_Mk}}\overset{P}{\longrightarrow} |\bar{X}^T|
\end{equation}\normalsize
...
\small\begin{equation*}
	\lim\limits_{k\rightarrow \infty}
	P(x^g=x^{T^*}|\mathbf{z}^{i}_{1:k})=1-\lim\limits_{k\rightarrow \infty}\sum\limits_{x^g\in \bar{X}^T}P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})=1
\end{equation*}\normalsize
%\begin{enumerate}
%	\item When $p_j\neq p^*_j$, i.e., $x^g\notin x^g_{eq}$,
%	\begin{align*}
%%		\lim\limits_{k\rightarrow \infty}\frac{1}{k}\ln\frac{P^i_{pdf}(x^g|\mathbf{z}^i_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})} & <0,
%		\sum\limits_{j=1}^{N}p^*_j\ln \frac{p_j}{p^*_j}+(1-p^*_j)\ln \frac{1-p_j}{1-p^*_j} & <0,
%		\;\text{thus }\\
%		\lim\limits_{k\rightarrow \infty}\frac{P^i_{pdf}(x^g|\mathbf{z}^i_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})} & =0
%	\end{align*}	
%	\item When $p_j= p^*_j$, i.e., $x^g\in x^g_{eq}$, \begin{align*}
%%	\lim\limits_{k\rightarrow \infty}\frac{1}{k}\ln\frac{P^i_{pdf}(x^g|\mathbf{z}^i_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})} & =0,
%	\sum\limits_{j=1}^{N}p^*_j\ln \frac{p_j}{p^*_j}+(1-p^*_j)\ln \frac{1-p_j}{1-p^*_j} & = 0,
%	\;\text{thus }\\
%	\lim\limits_{k\rightarrow \infty}\frac{P^i_{pdf}(x^g|\mathbf{z}^i_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^i_{1:k})} & =1
%	\end{align*}
%\end{enumerate}
\end{proof}
...
Here, the main idea is to classify UGV observation positions into two disjoint sets: \textit{infinite-observation spots} that contain positions where a UGV makes infinite observations as time tends to infinity, and \textit{finite-observation spots} that contain positions where the UGV makes finite observations.
...
%	the set of estimated target position of each UGV converges to $x^g_{eq}$ in probability using LIFO-DBF when the number of observations tends to infinity, i.e.,
	\small\begin{equation*}
		\lim\limits_{k\rightarrow \infty}P(x^g=x^{T^*}|\mathbf{z}^i_{1:k})=
%		\begin{cases}
%		1 & \text{if}\; i=1\\
%		0 & \text{if}\; i\neq 1
%		\end{cases},\;
...
\frac{P^i_{pdf}(x^g|\mathbf{z}^i_{1:k})}{P^i_{pdf}(x^{T^{*}}|\mathbf{z}^i_{1:k})}=\frac{P^i_{pdf}(x^g)\prod\limits_{j=1}^{N}\prod\limits_{l=1}^{k^i_j}P(z^j_l|x^g;x^R_l)}{P^i_{pdf}(x^{T^{*}})\prod\limits_{j=1}^{N}\prod\limits_{l=1}^{k^i_j}P(z^j_l|x^{T^{*}};x^R_l)}
\end{equation}\normalsize
...
This section simulates two scenarios of target localization to demonstrate the effectiveness of LIFO-DBF. 
...
The probabilistic sensor model, \cref{eqn:bin_sensor1,eqn:bin_sensor0}, takes the form of Gaussian functions \cite{bonnie2012modelling}:
%\small\begin{align}\label{eqn:gauss_sensor}
\small\begin{subequations}\label{eqn:gauss_sensor}
	\begin{align}
		P(z^i_k=1|x_k^g;x_k^{R,i})&=e^{-\frac{1}{2}(x_k^g-x_k^{R,i})^T{\Sigma}^{-1}(x_k^g-x_k^{R,i})}\\
%		\exp\left\lbrace -\frac{1}{2}(x^g-x^R)^T{\Sigma}^{-1}(x^g-x^R)\right\rbrace \\
		P(z^i_k=0|x_k^g;x_k^{R,i})&=1-P(z^i_k=1|x_k^g;x_k^{R,i}),
	\end{align}
\end{subequations}\normalsize
...
Six static UGVs, as represented by green starts and red square in \cref{fig:sta_sen_sta_tar_sing1}, are placed in the field.
...
The target motion is modeled as a single-integrator, where $A_k$ is an identity matrix and $B_k=\begin{bmatrix}
\Delta T & 0\\
0 & \Delta T
...
However, it is worth noting that, CbDF requires multiple rounds of exchanging individual PDFs, which incurs much higher communication burden than LIFO-DBF's approach of transmitting the communication buffer once at each time step.
...
This paper presents a measurement dissemination-based distributed Bayesian filtering (DBF) approach for a multi-UGV network, utilizing the Latest-In-and-Full-Out (LIFO) protocol for observation exchange.
...
The consistency of LIFO-based DBF is proved by utilizing the law of large numbers, which ensures that the estimated target position converges in probability to the true target position when the number of observations tends to infinity.
...
Imperfect communication, including package loss, out-of-order measurement and transmission delay, requires extension to LIFO-DBF and may affect its consistency properties.
