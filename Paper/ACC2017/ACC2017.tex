...
P(z^i_k=1|x^g_k;x^i_k)\in \left[0,1\right],\; x^g_k\in S
\end{equation}\normalsize
...
P(z^i_k=0|x^g_k;x^{i}_k)=1-P(z^i_k=1|x^g_k;x^{i}_k)
\end{equation}\normalsize
...
For the purpose of simplicity, we will not explicitly write $x^{i}_k$ when no confusion may occur.
...
This section proves the consistency of LIFO-DBF under a general assumption of sensors, i.e. a set of heterogeneous sensors. 
...
Considering $S$ is finite and $x^{g^*}$ is the true location of target, the consistency of LIFO-DBF for static UGVs is stated as follows: 
