...
%	 of $z^i_k$ given the sensor state $x^i_k$ and the target state $\xg_k$. 
	...
	For example, if $w^i_k$ is a zero-mean Gaussian white noise with covariance $\Gamma_k^i$, $P(z^i_k|x^g_k;x^i_k)$ can be described as $P(z^i_k|x^g_k;x^i_k)=N(h_i(\xg_k,x^i_k),\Gamma_k^i)$.
...
%	 of \small$G_{i_j},\,j=1\dots,l$\normalsize.
	Such collection is \textit{jointly strongly connected} if the union of its members forms a strongly connected graph\footnote{\textcolor{orange}{The counterpart definition for undirected graphs is given in \cite{jadbabaie2003coordination}.}}.
