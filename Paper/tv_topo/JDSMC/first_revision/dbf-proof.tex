...
%		The batch form of DBF at $k\thi$ step is 
		\textcolor{orange}{The DBF can be transformed into the batch form by recursively applying \Cref{eqn:bayes_upd} from $k$ to the initial time $1$ (back in time):}
		\textcolor{orange}{
		\small\begin{align*}
...
%		\end{equation*}\normalsize		
		...
		Due to the finite delay of measurement arrival (\Cref{cor1}), i.e., $i\thi$ UGV constantly receives the sensor measurements of all UGVs, we can use the law of large numbers to study the asymptotic behavior:}
