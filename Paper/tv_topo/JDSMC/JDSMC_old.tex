...
%	Let $\bar{G}=\left\lbrace G_1,G_2,\dots,G_L\right\rbrace $ denote the set of all possible simple and undirected graphs defined for the network of UGVs.
	...
	\section{Latest-In-and-Full-Out (\proto) Protocol}\label{sec:\proto}
	\todohere{include odometry information in transmitted message.}
	...
	Let $y_k^i=\left\lbrace x^i_k,z^i_k\right\rbrace$ and $Y^i_{a,b}=\left[ y_{a}^i,\dots,y_{b}^i\right]$.
	Under \proto, each UGV contains a communication buffer (CB) to store a subset of observations of all UGVs and the corresponding states of UGVs:
	\begin{equation*}		
		B^i_k=\left[ Y^1_{a^i_1,b^i_1},\dots,Y^1_{a^i_N,b^i_N}\right],
...
%	where $z^j_{k^i_j}$ represents the observation made by ${j^\text{th}}$ UGV at time $k^i_j$. 
	where $B^i_k$ is the CB of $i^\text{th}$ robot at time $k$.
	Note that under \proto and certain conditions of interaction topologies, $\mathcal{Q}_i=\left\lbrace 1,\dots,N\right\rbrace \setminus \left\lbrace i\right\rbrace$, which will be proved in \Cref{cor1}.
...
%		 $i^\text{th}$ and $j^\text{th}$ UGV.
	...
	\begin{cor}\label{cor1}
		With the same network condition in \Cref{prop1}, all elements in $\mathbf{z}^{CB,i}_k$ under \proto become filled within finite time, i.e., $\mathcal{Q}_i = \left\lbrace1,\dots,N \right\rbrace \setminus \left\lbrace i\right\rbrace $.	
...
%	$P(z^j_{k^i_j}|x^T)$ is the likelihood function of observation $z^j_{k^i_j}$, described in \Cref{eqn:bin_sensor1} and \Cref{eqn:bin_sensor0}.
	...
	%\small\begin{equation}\label{eqn:\proto-dbf-sta-tar}
	...
	%\subsection{Algorithm of \proto-DBF for Static Target}\label{subsec:\proto-dbf-sta-tar} 
	
	%This section derives the \proto-DBF algorithm for localizing a static target. 
	...
	%\small\begin{align}\label{eqn:\proto-dbf-sta-tar}
	...
	%\subsection{Algorithm of \proto-DBF for Moving Target}\label{subsec:\proto-dbf-mov-tar}
	%This section derives the \proto-DBF for localizing a moving target. 
	...
	%\cref{fig:\proto-DBF} illustrates the \proto-DBF procedure for the $1^\text{st}$ UGV as an example.
	...
	%The \textbf{\proto-DBF algorithm} for moving target is then stated in \cref{alg:lifo-dbf}.
	%
	%\begin{figure}%[thpb]
	%	\centering
	...
	This section presents the main result of this study that \proto-DBF achieves consistent and consensual estimation of target position provided that the union of interaction topologies across some time intervals are jointly connected frequently enough as the system evolves.
%	results of \proto-DBF under the condition of the interaction topology described in \cref{prop1}.
	To be specific, considering $S$ is finite and $x^{T^*}$ is the true position of target, the consistency of \proto-DBF for static UGVs is stated as follows:
	%\todohere{converge to a set or treat the PDF as a r.v.?}
	\begin{thm}\label{thm:\proto-dbf-sta-tar}
		Considering a network of $N$ static UGVs with the interaction topology condition in \cref{prop1}, the estimated target position converges to the true position of target in probability using \proto-DBF, i.e.,
	...
	\begin{proof}	
		...
		Utilizing the facts: (1) $z^j_l$ are conditionally independent samples from $P(z^j_l|x^{T^*})$ and (2) $k-(N-1)T_u<|\mathscr{K}^i_{j,k}|\leq k$, the law of large numbers yields
		
		\small\begin{subequations}\label{eqn:cmp_lim2}
			\begin{align}
			\frac{1}{k}\sum\limits_{l=1}^{k^i_j}\ln\frac{P(z^j_l|x^T)}{P(z^j_l|x^{T^*})}&\overset{P}{\longrightarrow}\mathbb{E}_{z^j_l} \left[\frac{P(z^j_l|x^T)}{P(z^j_l|x^{T^*})}\right]\\
			&=\int_{z^j_l} P(z^j_l|x^{T^*})\frac{P(z^j_l|x^T)}{P(z^j_l|x^{T^*})} dz^j_l\\
			&=-D_{KL}\left(P(z^j_l|x^{T^*})\|P(z^j_l|x^T)\right),
			\end{align}
		\end{subequations}\normalsize
		...
		\begin{subequations}
			\begin{align}
			\frac{1}{k}\sum\limits_{l=1}^{k^i_j}\ln\frac{P(z^j_l|x^T)}{P(z^j_l|x^{T^*})}<0,&\quad x^g\neq x^{T^*}\\
			\frac{1}{k}\sum\limits_{l=1}^{k^i_j}\ln\frac{P(z^j_l|x^T)}{P(z^j_l|x^{T^*})}=0,&\quad x^T= x^{T^*}.
			\end{align}
		\end{subequations}
		...
		\begin{subequations}
			\begin{align}
			\lim\limits_{k\rightarrow \infty}\frac{1}{k}\ln\frac{P^i_{pdf}(x^T|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})}<0,&\quad x^T\neq x^{T^*}\label{subeqn:limit1}\\
			\lim\limits_{k\rightarrow \infty}\frac{1}{k}\ln\frac{P^i_{pdf}(x^T|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})}=0,&\quad x^T= x^{T^*}\label{subeqn:limit2}.
			\end{align}
		\end{subequations}
		\Cref{subeqn:limit1,subeqn:limit2} imply that
		\begin{subequations}
			\begin{align}
			\frac{P^i_{pdf}(x^T|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})}\overset{P}{\longrightarrow}0,&\quad x^T\neq x^{T^*}\\
			\frac{P^i_{pdf}(x^T|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{T^*}|\mathbf{z}^{i}_{1:k})}\overset{P}{\longrightarrow}1,&\quad x^T= x^{T^*}.
			\end{align}
		\end{subequations}
		Therefore,
		\small\begin{equation*}
		\lim\limits_{k\rightarrow \infty}
		P(x^T=x^{T^*}|\mathbf{z}^{i}_{1:k})=1.
		\end{equation*}\normalsize		
...
%	\end{subequations}\normalsize
	...
	The information exchange among UGVs relies on the Latest-In-and-Full-Out (\proto) protocol, under which UGVs exchange full communication buffers with neighbors and can significantly reduce the transmission burden between each pair of UGVs to scale linearly with the network size.
	Under the condition that the union of undirected switching topologies is connected frequently enough,
	\proto can disseminate observations over the network within finite time. The \proto-based DBF algorithm is then derived to estimate individual probability density function (PDF) for target localization in a static environment. The consistency of this algorithm is proved by utilizing the law of large numbers, ensuring that each individual estimate of target position converges in probability to the true value.
