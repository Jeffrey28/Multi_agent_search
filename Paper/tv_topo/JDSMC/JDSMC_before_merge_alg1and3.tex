...
%%% first author
\author{Chang Liu\thanks{The first two authors have equally contributed to this research.}
	\affiliation{		
...
%	where $z^j_{k^i_j}$ represents the observation made by ${j^\text{th}}$ UGV at time $k^i_j$. 
	where $B^i_k$ is the CB of $i^\text{th}$ robot at time $k$ and $Y^j_{\K^{i,j}_k}$ represents the set of robot $j$'s measurements at time steps in $\K^{i,j}_k$ that are stored in robot $i$'s CB.
...
%	For the clarity of explanation of DBF in \cref{sec:\proto-dbf}, we define a \textit{new observation set} $\mathbf{z}^{new,i}_k$ for $i^\text{th}$ UGV to denote the set of observations that the $i^\text{th}$ UGV receives and stores in its CB at $k$.
	...
	\begin{algorithm}
		...
		\begin{algorithmic}
			...
			The CB of $i^\text{th}$ UGV is initialized at $k=0$:
			\begin{equation*}
				%$\begin{array}{c}
%				z^j_{k^i_j}=\varnothing,\; k^i_j=0,\;j=1,\dots,N
				B^i_0=\left[ Y^1_{\K^{i,1}_{0}},\dots,Y^N_{\K^{i,N}_{0}}\right],
%				\text{ where } Y^j_{\K^i_j} = \left\lbrace y^j_0\right\rbrace \text{ and } y^j_0 = \left\lbrace x^j_0,\varnothing\right\rbrace.
				\text{ where } Y^j_{\K^{i,j}_0} = \lb \left[ x^j_0,\varnothing\right]\rb .
				%\end{array}$\\
			\end{equation*}
			
			\State \textbf{(2)} At time $k^\text{th}\,(k\geq 1)$ for $i^\text{th}$ UGV:	
			
%			\State The interaction topology is represented by $G[k]\in\bar{G}$.
			\State (2.1) Receiving Step.
			
			The $i^\text{th}$ UGV receives all CBs of its direct neighborhood $\mathcal{N}_i(G_{k-1})$.
			The received CBs are totally $|\mathcal{N}_i(G_{k-1})|$ groups, each of which corresponds to the $(k-1)^\text{th}$ step CB of a UGV in $\mathcal{N}_i(G_ {k-1})$. 
			The received CB from $l^\text{th}$ UGV is
			\begin{equation*}
%				\mathbf{z}^{CB,l}_{k-1}=\left[ z^1_{(k-1)^l_1},\dots,z^N_{(k-1)^l_N}\right],\; l\in\mathcal{N}_i(G[k-1])
			...
			The $i^\text{th}$ UGV updates $Y^i_{\K^{i,i}_{k}}$ by its own state-measurement pair at current step:
			\begin{equation*}
...
%			\end{equation*}
			...
			The $i^\text{th}$ UGV updates other elements of its own CB, i.e., $Y^j_{\K^{i,j}_{k}}\,(j\neq i)$, by merging with all received CBs:
						
			For all $j\neq i$,
			\begin{equation*}
				Y^j_{\K^{i,j}_{k}} = Y^j_{\K^{i,i}_{k-1}} \cup Y^j_{\K^{l,j}_{k-1}},
				\forall\; l\in \mathcal{N}_i(G_{k-1}).
			\end{equation*}
%			\begin{align*}
...
%			\end{align*}
			...
			Trim the measurements in CB using the track list.
			See \Cref{alg:tracklist}.
			
			\State (2.4) Sending Step:
			
			The $i^\text{th}$ UGV broadcasts its updated CB to all of its neighbors defined in $\mathcal{N}_i(G_k)$.
			%\bottomrule
			%\end{tabular}
...
%	, which means under \proto each UGV has a maximum delay of 2 steps for receiving observations from other UGVs; 
	...
	\begin{prop}\label{prop1}
		%For a fixed and undirected network of $N$ UGVs, \proto uses the shortest path(s) between $i^\text{th}$ and $j^\text{th}$ UGV to exchange observation, the length of which is the delay $\tau_{i,j}$ between these two UGVs.
...
%		\todohere{may rephrase the following sentence}
		...
		then arbitrary pair of UGVs can exchange observations under \proto. In addition, the delay between each pair of UGVs is no greater than \small$(N-1)T_u$\normalsize, where \small$T_u=\sup\limits_{m=1,2,\dots}\left( k_{m+1}-k_m\right) T$ \normalsize is the upper bound of interval lengths.
	\end{prop}
	
	\begin{proof}		
		Consider the transmission from an arbitrary UGV $i$ to an arbitrary one $j$.
		Since each robot will receive neighbors' CBs and send the merged one to its neighbors in next step, UGV $i$ can transmit its information to $j$ if and only if there is a path between them, i.e.,
		\begin{equation*}
...
%		 $i^\text{th}$ and $j^\text{th}$ UGV.
		...
		Since the union of graphs across time interval $\left[k_1,k_2 \right)$ is jointly connected, $i^\text{th}$ UGV can directly communicate with at least one another UGV at a time instance, i.e., $\exists l_1\in V,\,t_1\in \left[k_1,k_2 \right)$ such that $i\in\mathcal{N}_{l_1}(G_{t_1})$.
%		This implies that observation $z^i_{t_1}$ is received and stored in the CB of $l_1^\text{th}$ UGV at $t_1+1$ under \proto.
		Therefore, at least one UGV other than $i^\text{th}$ UGV has received and received the CB from $i^\text{th}$ UGV by $k_2$.
		If $l_1=j$, then we have proved the exchange of CBs between $i$ and $j$.
...
%		For the former case, $z^i_{t_2}$ is received and stored in the CB of $l_2^\text{th}$ UGV at $t_2+1$ under \proto;
...
%		In both cases, at least two UGVs have received and stored an observation from $i^\text{th}$ UGV by $k_3$.
		Using similar derivation, it can be shown that all $N-1$ UGVs, except $i^\text{th}$ UGV, will receive and store the state-measurement pair from $i$ no later by $k_{N}$.
		Therefore, the transmission delay between an arbitrary pair of UGVs is no greater than \small$(N-1)T_u$\normalsize.
	\end{proof}
	%\vspace{\baselineskip}
	\medskip
	\begin{cor}\label{cor1}
		With the same network condition in \Cref{prop1}, all elements in CB under {\proto} become filled within finite time. This implies $\mathcal{Q}_i = \left\lbrace1,\dots,N \right\rbrace \setminus \left\lbrace i\right\rbrace $.	
	...
	\begin{proof}
		...
		\todohere{this argument seems incorrect.}
	...
	\section{Distributed Bayesian Filter via Full-In-and-Full-Out Protocol}\label{sec:\proto-dbf}
...
%	\begin{rem} 
%		The proposed \proto protocol 
%		%to be described in \cref{sec:\proto-dbf} 
%		is applicable for both homogeneous and heterogeneous binary sensors. 
%		A homogeneous model can simplify the analysis of \proto-based DBF, while a heterogeneous model is more close to real sensing characteristics.
%		In addition, it also works for other types of sensors, such as laser scanners and cameras.
%	\end{rem}	
	
%	\subsection{Distributed Bayesian Filter for Multiple UGVs}\label{subsec:dbf}
%	\todohere{use the tv version}
	
%	The distributed Bayesian filter (DBF) using \proto protocol is introduced in this section.
%	%, which is also stated in \cite{bandyopadhyay2014distributed} and \cite{julian2012distributed}. 
...
%	\end{align*}\normalsize
%	and $P^i_{pdf}(x^T|\mathbf{z}^{new,i}_{1:k})$ is called posterior individual PDF.
%
...
%	\subsection{Algorithm of LIFO-DBF for Moving Target}\label{subsec:LIFO-dbf-mov-tar}
	...
	This section derives the LIFO-DBF for localizing a target. 
	At time $k$, each UGV maintains an two PDFs, the stored PDF and the individual PDF.
	The stored PDF is used to represent the PDF that is updated from the initial PDf and all robots' measurements.
%	and a collection of measurement history, called the \textit{record set}. 
	The $i^\text{th}$ individual PDF is then computed from the stored PDF and the measurements in CB by running the Bayesian filter (\Cref{eqn:bayes_pred,eqn:bayes_upd}).
%	to obtain $P^i_{pdf}(\X_k|\mathbf{z}^{i}_{1:k})$.
	
	The \textbf{LIFO-DBF algorithm} is stated in \cref{alg:lifo-dbf}.
	...
	Note that we use $\Omega^i_{\xi}\,\left(\xi=1,\dots,N\right) $ to denote the index set of UGVs whose measurement at time $(k-N+\xi)$ is stored in $i^\text{th}$ UGV's CB, i.e. $\Omega^i_{\xi}=\left\lbrace j\in \mathcal{Q}_i \bigcup \left\lbrace i \right\rbrace | z^j_{k-N+\xi} \text{ is in the record set} \right\rbrace$.
	...
	\section{Track List for Reducing Communication Overhead}\label{sec:tracklist}
	The size of CBs can keep increasing as measurements cumulate over time. 
	To reduce the communication of unnecessary measurements in the CBs, we trim the measurements that have already been fused into the state estimation (stored PDF), as the Updating Step in Algorithm 1 shows.
	To avoid trimming the useful measurement, each robot keeps a \textit{track list}, $P^i_k=\left[\mathbf{p}^1_{k^{i,1}},\dots, \mathbf{p}^N_{k^{i,N}}\right]^T$,
	which is a binary list that represents the robot's knowledge of the oldest measurements (in terms of the measurement time) in all robots' CBs.
	$\mathbf{p}^j_{t_j}=\left[p^{j,l}_{t_j}, l\in\lb 1,\dots, N\rb\right]$ is a binary vector with size $N$.
	Each element, $p^{j,l}_t$, equals $1$ if the $i^\text{th}$ robot knows that the $\left[x^l_t,z^l_t\right]$ is in the $j^\text{th}$ robot's CB, and equals $0$ if the $i^\text{th}$ robot cannot determine whether $\left[x^l_t,z^l_t\right]$ is in the $j^\text{th}$ robot's CB.
	So it could happen that $\left[x^l_t,z^l_t\right]$ is in the $j^\text{th}$ CB but robot $i$ does not know this and thus $p^{j,l}_t=0$.
	When all elements of $P^i_t$ are $1$'s, it means that the robot $i$ can be sure that all robots have the measurements of time $t$ from all other robots and can thus fuse the measurements of time $t$ in its own CB to update the stored PDF. 
	Track lists are also exchanged when robots communicate and trim the corresponding measurements in CB. 
	\Cref{alg:tracklist} describes the exchange and updating of the track lists.
	
	The use of track lists can ensure that all information are used while avoiding overuse or OOSM.
	It also guarantees that fusing the measurements and trimming the CBs will not lose any information; they are encoded into the stored PDF.
	The following theorem proves this property.
	
	\begin{thm}\label{thm:trim_no_loss}
		Each robot will utilize all robots' measurements sequentially. The measurements trimmed from the CB are already fused in the stored PDF.
	\end{thm}
	
	\begin{proof}
		The design of track lists ensure that, measurements of certain times get trimmed only when all robots' CB have or have had them. Therefore, it causes no loss to trim theses measurements.
	\end{proof}
	
	Since the track list is used to reduce the size of CBs, it is necessary to understand how often the CBs will get trimmed.
	The following theorem gives the result.
	\begin{thm}\label{thm:upd_tl_freq}		
		The robot $i$ can trim the measurements of time $t$ at time $k_i=t+\max\limits_{l,j}\lb d_{lj}+d_{ji}\rb$, where $d_{lj}$ is the shortest path distance from robot $l$ to $j$. 
		Let $t^k_{l,j}=t+d_{lj}$ be the time that robot $l$ talks to $j$.
		Let $d_{ji}$ is the shortest path distance from robot $j$ to $i$, starting at time $t^k_{l,j}$.
		In other words, after $j$ gets message from $l$ at time $t^k_{l,j}$, it takes $d_{ji}$ steps for $j$ to let $i$ know that $j$ has got the $i$'s information of time $k$.		
	\end{thm}
	
	\begin{proof}
		\todohere{need to modify this proof.}
		Mathematically speaking, 
		\begin{gather*}
			\exists n_1, n_2 \in \mathbb{N}_+, \text{ s.t. } M^{n_1}_{l,j}>0 \text{ and } M^{n_2}_{j,i}>0.\\
			\text{Besides, } n=	n_1+n_2.
		\end{gather*}
	\end{proof}
	...
	\begin{cor}
		The first time to trim $i^\text{th}$ CB occurs at $k_i=1+\max\limits_{l,j}\lb d_{lj}+d_{ji} \rb.$
	\end{cor}
	
	\begin{proof}
		The proof is straightforward by setting $t=1$ in the proof of \Cref{thm:upd_tl_freq}.
	\end{proof}
	
	\begin{cor}
		The time between two consecutive trimming of $i^\text{th}$ CB is 
		\begin{equation*}
			\Delta k_i=\begin{cases}
			1+n'_i-n_i& \text{if}\;n'_i \ge n_i\\
			1& \text{if}\;n'_i < n_i
			\end{cases},
		\end{equation*}
		where $n_i=\max\limits_{l,j}\lb d_{lj}+d_{ji}\rb$ starting from $t$ and $n'_i=\max\limits_{l,j}\lb d_{lj}+d_{ji}\rb$ starting from $t+1$.
	\end{cor}
	
	\begin{proof}
		Let $k_i = t+1+n_i$ and $k'_i = t+n'_i$ be the two consecutive trimming times, where $t$ is the time of measurement that will be trimmed from the CB.
		Due to the rule of trimming, earlier measurements are always trimmed before that of later measurements. 
		This guarantees that $\Delta k \ge 1$. 
		If $n'_i \ge n_i$, then it takes longer time for later measurements to be trimmed and thus $\Delta k = 1+n_i'-n_i$.
	\end{proof}
	
	\begin{algorithm}
		\caption{Exchanging and Updating the Track List}
		\label{alg:tracklist}
		\begin{algorithmic}
			\State \textbf{(1)} Initialization.
			...
			The $i^\text{th}$ UGV receives all track lists of its direct neighborhood $\mathcal{N}_i(G_{k-1})$.
			The received track lists from $l^\text{th}$  $\left(l\in \mathcal{N}_i(G[k-1])\right)$ UGV is $P^l_{t_l}$.
%			\newline
			\State (2.2) Updating Track List.
			
			The $i^\text{th}$ UGV updates its own track list using all the received track lists:			
			...
			Trim the CB based on the updated track lists. Find the smallest time in the track list: $k_m=\min\lb k^{i,1},\dots,k^{i,N} \rb$. 			
			If all elements with the same time tag in $P^i_k$ are $1$'s, then 
			\begin{enumerate}
			\item set all these items in the track list to be $0$;
			\item remove all corresponding measurements in $i^\text{th}$ CB; 
			\item increase all time tags equivalent to $k_m$ by $1$.
			\item update the track list with items in current CB.
			\end{enumerate}
				
			\State (2.4) Sending Step.	
			The $i^\text{th}$ UGV broadcasts its updated track list to all of its neighbors defined in $\mathcal{N}_i(G_k)$.
...
%		\end{equation*}\normalsize
	...
	\begin{proof}	
		For the purpose of clarity, define the time set of $i^\text{th}$ UGV, $\mathscr{K}^{i}_{j,k},\,j\in\left\lbrace 1,\dots,N\right\rbrace $, that contains the time steps of measurement by $j^\text{th}$ UGV that are contained in $B^i_k$.
		...
		Considering the conditional independence of measurements given $x^g_k\in S$, the batch form of DBF at $k^\text{th}$ step is
...
%	\medskip
	...
	%Similar to \Cref{thm:\proto-dbf-sta-tar}, the batch form of DBF at $k^\text{th}$ step is
