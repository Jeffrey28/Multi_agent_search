...
%	\footnote{\todohere{change notations here} Due to the space limit, in this figure we use $P^i_{pdf}(k)$, $P^i_{pdf}(k-N)$ and $P^i_{pdf}(k-N+1)$ to represent $P^i_{pdf}(\X|\mathbf{z}^{i}_{1:k})$ $P^i_{pdf}(\X|\mathbf{z}^{i}_{1:k-N})$ and $P^i_{pdf}(\X|\mathbf{z}^{i}_{1:k-N+1})$, respectively.}.
	...
	\begin{algorithm}
		...
		\label{alg:upd_tl}
		\begin{algorithmic}
			\State Consider updating the $i\thi$ UGV's TL using the received $j\thi$ UGV's TL.
			...
			\State \textbf{(3)} Update other rows of TL:
			$\forall l\in \mathcal{N}_i(G_{k-1})\setminus\lb j\rb,\,\forall r\in \lb 1\dots,N\rb,$
			\begin{itemize} 
...
%	When all elements of $\Q^i_k$ are $1$'s, the $i\thi$ UGV can be sure that the $j\thi$ UGV ($\forall j\in V$) has received the state-measurement pairs of time $k^{i,j}$ from all UGVs.
	...
	The exchange and updating of TLs are described in the TL part in \Cref{alg:lifo,alg:upd_tl}.
	...
	\todohere{add a plot showing the change of TL.}
	\todohere{think how to illustrate the trim process?}
...
%		, i.e., $\left[x^l_{k_m},z^l_{k_m}\right],\,l\in V$. 
	...
	Under the \fc ness condition, it can be derived that the size of each UGV's CB is upper bounded.
	\begin{thm}
	...
	The size of the CB is upper bounded by $O(NT_u)$, according to \Cref{thm:max_CB_size}, and the size of the TL is $O(N^2)$.
