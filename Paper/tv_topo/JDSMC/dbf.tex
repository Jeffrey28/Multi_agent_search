...
%	\end{subequations*}
	...
	\begin{algorithm}
		...
		\end{algorithmic}
	\end{algorithm}
	\footnotetext{This step will be modified in \Cref{subsec:tracklist} after introducing the track list.}
	...
	Another possible remedy is to fuse all measurements by running the filtering algorithm from the initial individual PDF at every time step.
	This, however, causes excessive computational burden.
	To avoid the OOSM problem and unnecessary computational complexity, we add a new PDF, namely the \textit{stored PDF}, $P^{i}_{sto}(\X_t)$, that is updated from the $i\thi$ UGV's initial PDF by fusing the state-measurement pairs of \textit{all} UGVs up to a certain time $t\le k$.
...
%		\vspace{-1em}
	...
	\begin{algorithm}
		...
		\label{alg:upd_tl}
		\begin{algorithmic}
			\State Consider updating the $i\thi$ UGV's TL using the received $l\thi$ UGV's TL. 			
			\State \textbf{(1)} Update the $i\thi$ row of TL using the $i\thi$ UGV's CB (\Cref{eqn:tl_entry}).
			%			\State \textbf{(2)} Update the $j\thi$ row of TL using the received $j\thi$ UGV's CB $\B^j_{k-1}$.
			\State \textbf{(2)} Update other rows of TL:
			$\forall r\in \mathcal{N}_i(G_{k-1}),$ %$,\,\forall r\in \lb 1,\dots,N\rb,$
			\begin{itemize} 
				\item if $k^{ir}>k^{lr}$, keep current $\mathbf{q}^{ir}_{k^{ir}}$;
				\item if $k^{ir}=k^{lr}$, $\mathbf{q}^{ir}_{k^{ir}}=\mathbf{q}^{ir}_{k^{ir}} \lor\footnotemark\mathbf{q}^{lr}_{k^{lr}}$; 
				\item if $k^{ir}<k^{lr}$, $\mathbf{q}^{ir}_{k^{ir}}=\mathbf{q}^{lr}_{k^{lr}}$ and $k^{ir}=k^{lr}$.
			\end{itemize}
		\end{algorithmic}
	\end{algorithm}
...
%		\vspace{-1em}
	...
	Therefore, a TL can be represented by a binary matrix with the size $N\times (N+1)$.
	$\Q^i_k$ represents the $i\thi$ UGV's knowledge of its own and other UGVs' measurements of the times specified by $k^{ij}$'s: %, in terms of the measurement time,
	...
	It is guaranteed by the updating rule that the $i\thi$ UGV knows that the $j\thi$ UGV has received the state-measurement pairs of time earlier that $k^{ij}$ from any other UGVs.
...
%	\todohere{rethink about how many steps a TL can be trimmed.}
	...
	The exchange and updating of TLs are described in the TL parts in \Cref{alg:lifo}, with the details of the updating presented in \cref{alg:upd_tl}.
%	It can be noticed that, a TL keeps track of the earliest time of state-measurement pairs that the corresponding UGV does not have.
...
%		According to \Cref{cor1}, each UGV receives the state-measurement pairs from all other UGVs within finite time steps that are used for updating the individual PDF.
		...
		Trimming $Q^i_k$ happens when all entries corresponding to time $k^i_m$ equal $1$. 
...
%		, i.e., $\left[x^l_{k_m},z^l_{k_m}\right],\,l\in V$. 
		A UGV has either saved the pairs in its CB or already fused them to obtain the stored PDF.
...
%	\end{proof}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}
			...
			\begin{enumerate}
				\item set all these items in the track list to be $0$, let $k^i_m \leftarrow k^i_m+1.$
