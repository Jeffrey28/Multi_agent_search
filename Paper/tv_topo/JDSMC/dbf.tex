...
%	\end{subequations*}
	...
	The individual PDF, $P^i_{pdf}(\X_k|\zb^i_{1:k})$, is then computed by fusing the measurements from time $t+1$ to $k$ in the CB into $P^i_{sto}(\X_t)$, running the Bayesian filter (\Cref{eqn:bayes_pred,eqn:bayes_upd}).
...
%	When all elements of $\Q^i_k$ are $1$'s, the $i\thi$ UGV can be sure that the $j\thi$ UGV ($\forall j\in V$) has received the state-measurement pairs of time $k^{i,j}$ from all UGVs.
	...
	\Cref{fig:} shows the trim of a CB.	
...
%		, i.e., $\left[x^l_{k_m},z^l_{k_m}\right],\,l\in V$. 
	...
	%	Under the \fc ness condition, it can be derived that the CB of each UGV is upper bounded.
	%	\begin{thm}
	%		The length of the $i\thi$ UGV's CB is no greater than $2(N-1)T_u$.
	%	\end{thm}
	%	
	%	\begin{proof}
	%		Consider the time intervals $\left[k_m,k_{m+1} \right),\,m=1,2,\dots$ that are {\fc}. 
	%		By $k_3$, each UGV has received all other UGVs' measurements of $\left[k_1,k_2 \right)$, and by $k_4$, the times of each UGV's track list has been updated to no less that $k_2$.
	%	\end{proof}
	...
	\todohere{this part requires the analysis of the length of CB. needs to work on this.}
