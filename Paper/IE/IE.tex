...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\todohere{consider using "measurement" instead of "observations" as the main term.}
...
As a generic filtering scheme for general system dynamics and arbitrary noise distributions, distributed Bayesian filters (DBF) have received increasing interest during past years \cite{bandyopadhyay2014distributed,julian2012distributed} and is the focus of this study.
...
Each UGV is equipped with a noisy sensor for environmental perception. 
Due to the limit of communication range, each UGV can only share information locally by exchanging measurements with its neighbors. 
The Bayesian filter is run locally on each UGV based on its own and received measurements via single-hopping to estimate true position of the target.
...
\todohere{this may make people wonder if LIFO-DBF can be applied to system with noise?}
\begin{equation}
\small
...
Each UGV constantly measures the target position and the sensor measurement of $i^\text{th}$ UGV can be described by a stochastic model:
\begin{equation}
...
We use a likelihood function to represent the probability of the target being detected by a sensor:
...
\begin{equation}
	p^i_{1,k}\sim
	\begin{cases}
		\mathcal{N}(\bar{z}^i_k,\Sigma^i_k) & \text{if}\,x^g_k\in\mathcal{F}(y^i_k)\\
		0 & \text{if}\,x^g_k\in\mathcal{F}(y^i_k),
	\end{cases}
\end{equation}
where $\bar{z}^i_k$ is the nominal value of the measurement and equals $\|x^g_k-x^i_k\|$, $\measuredangle(x^g_k-x^i_k)$ and $x^g_k-x^i_k$ for range-only, bearing-only and range-bearing sensors, correspondingly.
...
%P(z^i_k|x^g;x^i_k)=(p^i_{1,k})^{z^i_k}(p^i_{0,k})^{1-z^j_l}.
%\end{equation*}\normalsize
% parameterized by $x^g_k$ and $x^i_k$. 

For the purpose of simplicity, we will not explicitly write $x^{i}_k$ in the sensor model for the rest of the paper.
...
\begin{rem} 
	The proposed LIFO protocol to be described in \cref{sec:LIFO-dbf} and the consistency property proved in \Cref{sec:consist_proof} are applicable for general sensors, not limited to the ones described in this section. In addition, they also do not rely on the Gaussian noise assumption.
...
$P^i_{pdf}(x^g_{k}|\mathbf{z}^i_{1:k})$, where $\mathbf{z}^i_{1:k}$ denotes the set of observations by $i^\text{th}$ UGV and by UGVs in $\mathcal{Q}_i$ that are transmitted to $i^\text{th}$ UGV by time $k$.
...
The individual PDF is initialized as $P^i_{pdf}(x^g_0|\mathbf{z}^i_0)=P(x^g_0)$,
...
For the deterministic target motion model, as defined in \Cref{eqn:tar_motion_model}, its Markov motion model is simplified to be
...
\end{align*}\normalsize
and $P^i_{pdf}(x^g_k|\mathbf{z}^i_{1:k})$ is called posterior individual PDF; $P(z^i_k|x^g_k)$ is the likelihood function of observation $z^i_k$, described in \Cref{eqn:bin_sensor1} and \Cref{eqn:bin_sensor0}.
...
%\begin{rem}
	...
	To be specific, consider an $M\times M$ grid environment with a network of $N$ UGVs, the transmitted data of LIFO between each pair of UGVs are only the CB of each UGV and the corresponding UGV positions where observations were made, the size of which is $O(N)$, scaling linearly with UGV number. 
...
For simplicity and clarity, we assume $S$ is a finite set (e.g. a finely discretized field) and $x^{g^*}$ is the true position of target.
...
%Assume $S$ is finite (e.g. a finely discretized field) and $x^{g^*}$ is the true position of target, 
...
%	P_{pdf}^i(x^g=x^{g^*}|\mathbf{z}^{i}_{1:k})=
%	1,\;i=1,\dots,N.
%	\end{equation*}\normalsize
	\small\begin{equation*}
	\lim\limits_{k\rightarrow \infty}
	P(x_{MAP}=x^{g^*}|\mathbf{z}^{i}_{1:k})=
	1,\;i=1,\dots,N,
	\end{equation*}\normalsize
	where 
	\small\begin{equation*}
	x_{MAP}=\arg\max\limits_{x^g}P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k}).
	\end{equation*}
\end{thm}
...
The decomposition into multiplication in \Cref{eqn:bayes_batch2} results from the conditional independence of observations given $x^g$.
...
Since $P^i_{pdf}(x^g)$ and $P^i_{pdf}(x^{g^*})$ are bounded, then
\small\begin{equation}\label{eqn:cmp_lim1}
\lim\limits_{k\rightarrow \infty}\frac{1}{k}\ln\frac{P^i_{pdf}(x^g)}{P^i_{pdf}(x^{g^*})}= 0.
\end{equation}\normalsize

Utilizing the facts: (1) $z^j_l$ are conditionally independent samples from $P(z^j_l|x^g)$ and (2) $k-N<k^i_j\leq k$, the law of large numbers yields
%\small\begin{equation*}
...
\begin{align}
\frac{1}{k}\sum\limits_{l=1}^{k^i_j}\ln\frac{P(z^j_l|x^g)}{P(z^j_l|x^{g^*})}&\overset{P}{\longrightarrow}\mathbb{E}_{z^j_l} \left[\ln\frac{P(z^j_l|x^g)}{P(z^j_l|x^{g^*})}\right]\\
&=\int_{z^j_l} P(z^j_l|x^{g^*})\ln\frac{P(z^j_l|x^g)}{P(z^j_l|x^{g^*})} dz^j_l\\
&=-D_{KL}\left(P(z^j_l|x^{g^*})\|P(z^j_l|x^g)\right),
\end{align}
\end{subequations}\normalsize
...
The KL divergence has the property that
\begin{subequations}
	\begin{gather*}
...
%\begin{subequations}
%	\begin{align}
%		\frac{1}{k}\sum\limits_{l=1}^{k^i_j}\ln\frac{P(z^j_l|x^g)}{P(z^j_l|x^{g^*})}<0,&\quad x^g\neq x^{g^*}\\
%		\frac{1}{k}\sum\limits_{l=1}^{k^i_j}\ln\frac{P(z^j_l|x^g)}{P(z^j_l|x^{g^*})}=0,&\quad x^g= x^{g^*}.
%	\end{align}
%\end{subequations}

\begin{equation}
	\lim\limits_{k\rightarrow \infty}\frac{1}{k}\sum\limits_{l=1}^{k^i_j}\ln\frac{P(z^j_l|x^g)}{P(z^j_l|x^{g^*})}\;
	\begin{cases}
		<0,&\quad x^g\neq x^{g^*},\\
		=0,&\quad x^g= x^{g^*}.
	\end{cases}
...
%\begin{subequations}
%	\begin{align}
%	\lim\limits_{k\rightarrow \infty}\frac{1}{k}\ln\frac{P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{g^*}|\mathbf{z}^{i}_{1:k})}<0,&\quad x^g\neq x^{g^*}\label{subeqn:limit1}\\
%	\lim\limits_{k\rightarrow \infty}\frac{1}{k}\ln\frac{P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{g^*}|\mathbf{z}^{i}_{1:k})}=0,&\quad x^g= x^{g^*}\label{subeqn:limit2}.
%	\end{align}
%\end{subequations}

\begin{equation}\label{subeqn:limit}
	\lim\limits_{k\rightarrow \infty}\frac{1}{k}\ln\frac{P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{g^*}|\mathbf{z}^{i}_{1:k})}\;
	\begin{cases}
		<0,&\quad x^g\neq x^{g^*},\\
		=0,&\quad x^g= x^{g^*}.
	\end{cases}	
...
%\begin{subequations}
%	\begin{align}
%	\frac{P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{g^*}|\mathbf{z}^{i}_{1:k})}\overset{P}{\longrightarrow}0,&\quad x^g\neq x^{g^*}\\
%	\frac{P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{g^*}|\mathbf{z}^{i}_{1:k})}\overset{P}{\longrightarrow}1,&\quad x^g= x^{g^*}.
%	\end{align}
%\end{subequations}

\begin{equation}
	\frac{P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})}{P^i_{pdf}(x^{g^*}|\mathbf{z}^{i}_{1:k})}\overset{P}{\longrightarrow}
	\begin{cases}
		0,&\quad x^g\neq x^{g^*},\\
		1,&\quad x^g= x^{g^*}.
	\end{cases}
...
\small\begin{equation*}
\lim\limits_{k\rightarrow \infty}
P_{pdf}^i(x^g=x^{g^*}|\mathbf{z}^{i}_{1:k})=1.
\end{equation*}\normalsize
It is then straightforward to show
\begin{equation*}
\lim\limits_{k\rightarrow \infty}
P(x_{MAP}=x^{g^*}|\mathbf{z}^{i}_{1:k})=1.
\end{equation*}
...
%
%Since $c_M<0$,  ${P^i_{pdf}(x^{g^*}|\mathbf{z}^{i}_{1:k})}e^{c_Mk}{\longrightarrow}0$, \Cref{eqn:lim3} implies
%\begin{equation}\label{eqn:lim4}
%\sum\limits_{x^g\in \bar{X}^T}P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})e^{\left[c_M-c(x^g)\right]k}\overset{P}{\longrightarrow}0.
%\end{equation}
%
%Utilizing the relation 
%\begin{equation*}
%0\leq P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})\leq P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})e^{\left[c_M-c(x^g)\right]k},
%\end{equation*}
...
\begin{thm}\label{thm:LIFO-dbf-mov-tar}
	...
	\small\begin{equation*}
	\lim\limits_{k\rightarrow \infty}P(x_{MAP}=x_k^{g^*}|\mathbf{z}^i_{1:k})=
	1,\;i=1,\dots,N.
%	P(x_k^g=x_k^{g^*}|\mathbf{z}^i_{1:k})=1,\;i=1,\dots,N.
	\end{equation*}\normalsize
\end{thm}
\begin{proof}
	Since the target motion model is deterministic, as defined in \Cref{eqn:markov_model}, the integration in the prediction step (\Cref{eqn:bayes_pred}) can be removed, as no uncertainty exists in $P(x_k^g|x^g_{k-1})$.
	
	\Cref{eqn:bayes_batch} now becomes
	\small\begin{subequations}
		\begin{align*}
		P^i_{pdf}(x_k^g|\mathbf{z}^{i}_{1:k})&=P^i_{pdf}(x_k^g|z^1_{1:k^i_1},\dots,z^N_{1:k^i_N})\\
		&=\frac{P^i_{pdf}(x_0^g)\prod\limits_{j=1}^{N}\prod\limits_{l=1}^{k^i_j}P(z^j_l|x^g_l)P(x^g_l|x^g_{l-1})}{\sum\limits_{x^g_k\in S}P^i_{pdf}(x^g_0)\prod\limits_{j=1}^{N}\prod\limits_{l=1}^{k^i_j}P(z^j_l|x^g_l)P(x^g_l|x^g_{l-1})}.
		\end{align*}
	\end{subequations}\normalsize
	The updating terms $P(x^g_l|x^g_{l-1})$ cancel when comparing $P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})$ with $P^i_{pdf}(x^{g^*}|\mathbf{z}^{i}_{1:k})$, yielding the same result as \Cref{eqn:cmp}. Therefore, the rest of the proof is similar to that of \Cref{thm:LIFO-dbf-sta-tar}.
\end{proof}
...
The difficulty of consistency proof for this case lies in the fact that each UGV makes observations at multiple positions.
Here, the main idea of proof is to classify UGV observation positions into two disjoint sets: \textit{infinite-observation spots} that contain positions where a UGV makes infinitely many observations as time tends to infinity, and \textit{finite-observation spots} that contain positions where the UGV makes finitely many observations (i.e. the positions where the UGV never visits again after a finite time period).
...
\begin{thm}\label{thm:LIFO-dbf-mov-sen}
	...
	\small\begin{equation*}
		\lim\limits_{k\rightarrow \infty}
		P(x_{MAP}=x^{g^*}|\mathbf{z}^i_{1:k})=1,\;i=1,\dots,N.
%		P(x^g=x^{g^*}|\mathbf{z}^i_{1:k})=1,\;i=1,\dots,N.
	\end{equation*}\normalsize
\end{thm}
...
Therefore, \Cref{eqn:cmp2} can be reduced to only considering infinite-observation spots and the rest of proof is similar to that of \Cref{thm:LIFO-dbf-sta-tar}.
\end{proof}

\begin{rem}
	The assumption of unbiased sensors are important for the consistency. In fact, with unknown non-zero bias, the distribution of $z^j_l$ differs from $P(z^j_l|x^{g^*})$, which invalidates the derivation in \Cref{eqn:cmp_lim2} and the consistency proof. 
	This assumption also makes intuitive sense.
	In the extreme case, if each sensor has a very large measurement offset, then the estimated target position of each sensor (without communicating with other sensors) will be very different from each other's.
	Therefore, no common target position can be obtained when they fuse measurement from other sensors.
\end{rem}
\medskip
%\begin{rem}
%%	When $x^g_{eq}$ only contains $x^{T^{*}}$, consistency means the estimated target position converges to true target position in probability. Additionally, 
...
\section{Simulation and Experiment}\label{sec:sim}
\todohere{This section needs a major revision. also notice to simplify the description of each plot.}

\begin{figure}%[thpb]
	\centering
	\begin{subfigure}[b]{0.21\textwidth}
		\includegraphics[width=\textwidth]{figures/brg_sta_sen_sta_tar_rbt3_step1}
		\caption{Step 1}\label{fig:sta_sen_sta_tar_sing1}
	\end{subfigure}
	\begin{subfigure}[b]{0.21\textwidth}
		\includegraphics[width=\textwidth]{figures/brg_sta_sen_sta_tar_rbt3_step5}
		\caption{Step 5}\label{fig:sta_sen_sta_tar_sing2}
	\end{subfigure}
	\begin{subfigure}[b]{0.21\textwidth}
		\includegraphics[width=\textwidth]{figures/brg_sta_sen_sta_tar_rbt3_step10}
		\caption{Step 10}\label{fig:sta_sen_sta_tar_sing3}
	\end{subfigure}
	\begin{subfigure}[b]{0.21\textwidth}
		\includegraphics[width=\textwidth]{figures/brg_sta_sen_sta_tar_rbt3_step30}
...
\begin{figure}%[thpb]
	...
	\end{subfigure}
	\begin{subfigure}[b]{0.21\textwidth}
		\includegraphics[width=\textwidth]{figures/hetero_sta_sen_sta_tar_rbt5_step30}
		\caption{Step 30}%\label{fig:sta_sen_sta_tar_sing4}
	\end{subfigure}
	\begin{subfigure}[b]{0.23\textwidth}
...
Simulation has been conducted to demonstrate the effectiveness of LIFO-DBF for target localization using a team of six UGVs.
%In all scenarios, each UGV is equipped with a binary sensor. 
The networked UGVs take a ring communication topology that each UGV can communicate with two fixed nearest neighbors.
...
DBF is implemented using grid-based method, by which the space is finely discretized \cite{}. 
...
In both scenarios, LIFO-DBF is compared with two commonly adopted approaches in multi-agent filtering: the consensus-based distributed filtering (CbDF) method and the centralized filtering (CF) method.
...
Multiple rounds of communication and averaging are conducted during the ``Sending Step'' and ``Receiving Step'' in \Cref{alg:lifo} at each round to ensure the convergence of each UGV's individual PDFs.
...
We assume all UGVs are equipped with bearing-only sensors.
\subsubsection{Homogeneous Sensors}\label{subsec:sim1}
Six static UGVs, as represented by green stars and red square in \cref{fig:sta_sen_sta_tar_sing1}, are placed in the field.
...
LIFO-DBF is compared with CbDF and CF, results of which are presented in \cref{fig:sta_sen_sta_tar_pos_err,fig:sta_sen_sta_tar_entropy}.
...
Six UGVs are equipped with heterogeneous sensors, with two having range-only sensors, two having bearing-only sensors and two having range-bearing sensors.
...
The target motion is modeled as a single-integrator with process noise.
%, where $A_k$ is an identity matrix and $B_k=\begin{bmatrix}
%\Delta T & 0\\
%0 & \Delta T
%\end{bmatrix},$ %$u=\begin{bmatrix}v(1)\\v(2)\end{bmatrix}$,%\left[v(1);v(2)\right]^T$.
%\small$x^g(k+1)=x^g(k)+v\Delta T$\normalsize,
...
\cref{fig:mov_sen_mov_tar_sing1,fig:mov_sen_mov_tar_sing2,fig:mov_sen_mov_tar_sing3,fig:mov_sen_mov_tar_sing4} illustrates the test trial, in which
%when the UGVs move in circles.
...
%The target dynamics is given by a single-integrator model: $x^g(k+1)=x^g(k)+v\Delta T$,
%\small\begin{equation*}
%x^g(k+1)=x^g(k)+v\Delta T
%\end{equation*}\normalsize
...
\cref{fig:mov_sen_mov_tar_pos_err,fig:mov_sen_mov_tar_entropy} compares LIFO-DBF with CbDF and CF.
...
However, CbDF achieves comparable position estimation error as the CF, which are marginally better than LIFO-DBF.
...
However, it is worth noting that, CbDF requires multiple rounds of exchanging individual PDFs, which incurs much higher communication burden than LIFO-DBF at each time step.
...
\begin{figure}%[thpb]
	\centering
	\begin{subfigure}[b]{0.21\textwidth}
		\includegraphics[width=\textwidth]{figures/brg_mov_sen_mov_tar_rbt6_step1}
		\caption{Step 1}\label{fig:mov_sen_mov_tar_sing1}
	\end{subfigure}
	\begin{subfigure}[b]{0.21\textwidth}
		\includegraphics[width=\textwidth]{figures/brg_mov_sen_mov_tar_rbt6_step5}
		\caption{Step 5}\label{fig:mov_sen_mov_tar_sing2}
	\end{subfigure}	
	\begin{subfigure}[b]{0.21\textwidth}
		\includegraphics[width=\textwidth]{figures/brg_mov_sen_mov_tar_rbt6_step10}
		\caption{Step 10}\label{fig:mov_sen_mov_tar_sing3}
	\end{subfigure}	
	\begin{subfigure}[b]{0.21\textwidth}
		\includegraphics[width=\textwidth]{figures/brg_mov_sen_mov_tar_rbt6_step30}
	...
	%	\end{subfigure}
	\caption{(a)-(d) The $6^\text{th}$ UGV's individual PDFs at different times. (e) Average position estimation errors. (f) Average entropy of individual PDFs.}
...
\begin{figure}%[thpb]
	...
	\end{subfigure}
	\begin{subfigure}[b]{0.21\textwidth}
		\includegraphics[width=\textwidth]{figures/hetero_mov_sen_mov_tar_rbt4_step30}
		\caption{Step 30}%\label{fig:sta_sen_sta_tar_sing4}
	\end{subfigure}
	\begin{subfigure}[b]{0.23\textwidth}
...
Two types of LIFO-based DBF algorithms are proposed to estimate individual PDFs for a static target and a moving target, respectively. 
...
The consistency of LIFO-based DBF is proved by utilizing the law of large numbers, which ensures that the estimated target position converges in probability to the true target position.
% when the number of observations tends to infinity.
...
Future work includes considering other types of sensors and imperfect communication between UGVs.
