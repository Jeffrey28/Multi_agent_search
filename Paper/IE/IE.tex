...
\todohere{mention the relation between BF and KF, PF}
...
This section proves the consistency of LIFO-DBF under general types of sensors, i.e. a set of heterogeneous sensors.
Consistency is an important evaluation metric for stochastic filtering approaches \cite{}.
It differs from the concept of consensus: the latter implies that the estimation results of all sensors are the same; the former not only implies achieving consensus asymptotically, but also requires the estimated position converges to the true target position.
...
The consistency for moving UGVs and moving target are then consecutively proved.
For simplicity and clarity, we assume $S$ is finite and $x^{g^*}$ is the true position of target.
\todohere{maybe try to find a term in literature that describes the non-uniqueness of target position for range-only or bearing-only sensors with 360 degree FOV.}
...
Assume $S$ is finite (e.g. a finely discretized field) and $x^{g^*}$ is the true position of target, the consistency of LIFO-DBF for static UGVs is stated as follows: 
%\todohere{converge to a set or treat the PDF as a r.v.?}
\begin{thm}\label{thm:LIFO-dbf-sta-tar}
	Assume UGVs are static and the sensors are unbiased, the estimated target position converges to the true position of target in probability using LIFO-DBF, i.e.,
...
\begin{proof}	
Considering the conditional independence of observations given $x^g\in S$, the batch form of DBF at $k^\text{th}$ step is:
...
Utilizing the facts: (1) $z^j_l$ are conditionally independent samples from $B(1,p^{j^*}_{1})$ and (2) $k-N<k^i_j\leq k$, the law of large numbers yields
%\small\begin{equation*}
...
\begin{align}
\frac{1}{k}\sum\limits_{l=1}^{k^i_j}\ln\frac{P(z^j_l|x^g)}{P(z^j_l|x^{g^*})}&\overset{P}{\longrightarrow}\mathbb{E}_{z^j_l} \left[\frac{P(z^j_l|x^g)}{P(z^j_l|x^{g^*})}\right]\\
&=\int_{z^j_l} P(z^j_l|x^{g^*})\frac{P(z^j_l|x^g)}{P(z^j_l|x^{g^*})} dz^j_l\\
...
\begin{thm}\label{thm:LIFO-dbf-mov-tar}
	Assume UGVs are static, sensors are unbiased and the target is moving deterministically, the estimated target position converges to the true position of the target in probability using LIFO-DBF, i.e.,
	\small\begin{equation*}
	\lim\limits_{k\rightarrow \infty}P(x_k^g=x_k^{g^*}|\mathbf{z}^i_{1:k})=
	%		\begin{cases}
	%		1 & \text{if}\; i=1\\
	%		0 & \text{if}\; i\neq 1
	%		\end{cases},\;
...
\begin{proof}
	...
	\small\begin{subequations}
		...
		\end{align*}
	\end{subequations}\normalsize
	The updating terms $P(x^g_i|x^g_{i-1})$ in nominator and denominator cancel when comparing $P^i_{pdf}(x^g|\mathbf{z}^{i}_{1:k})$ with $P^i_{pdf}(x^{g^*}|\mathbf{z}^{i}_{1:k})$, yielding the same result as \Cref{eqn:cmp}. Therefore, the rest of the proof is similar to that of \Cref{thm:LIFO-dbf-sta-tar}.
\end{proof}
...
Here, the main idea is to classify UGV observation positions into two disjoint sets: \textit{infinite-observation spots} that contain positions where a UGV constantly makes observations as time tends to infinity, and \textit{finite-observation spots} that contain positions where the UGV makes finite observations (i.e. the positions where the UGV never visits again after a finite time period).
...
\begin{thm}\label{thm:LIFO-dbf-mov-sen}
	Assume UGVs move within a collection of finite positions and sensors are unbiased, the estimated target position converges to the true position of target in probability using LIFO-DBF, i.e.,
	\small\begin{equation*}
...
\begin{proof}
	Without loss of generality, we assume the target is static. The proof when target is moving is similar.
...
For the former, by referring to \Cref{eqn:lim} in proof of \Cref{thm:LIFO-dbf-sta-tar}, it is easy to know that their contribution to \Cref{eqn:cmp2} diminishes when $k\rightarrow \infty$.
...
\begin{rem}
	The assumption of unbiased sensors are important for the consistency. In fact, with unknown bias, the distribution of $z^j_l$ differs from $P(z^j_l|x^{g^*})$, which invalidates the derivation in \Cref{eqn:cmp_lim2} and thus the consistency proof. 
	This assumption also makes intuitive sense.
	In the extreme case, if each sensor has a very large measurement offset, then the estimated target position of each sensor (without communicating measurement with other sensors) will be very different from each other's. 
