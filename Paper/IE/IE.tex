...
For a static target, each UGV locally fuses the newly received measurements while for a moving target, a set of historical measurements is stored and sequentially fused. 
...
Leung et al. (2010) proposed a decentralized state-estimation algorithm for dynamic robot networks \cite{leung2010decentralized}.
The algorithm was shown to achieve centralized-equivalent filtering performance via simulations, no theoretical guarantee about the estimation consistency was provided. 
...
\begin{figure}
	\centering
	\includegraphics[width=0.39\textwidth]{figures/exp_scene2}
	\caption{Experiment Setup. A P3DX robot (red) is equipped with a sonar for localizing the target (black box). The room size is $9.09m\times 6.37m$.}\label{fig:exp_scene}
\end{figure}
...
\end{figure}
We have conducted an experiment to validate the LIFO-DBF approach using sonar sensors mounted on a P3DX robot platform for localizing a static target (\Cref{fig:exp_scene}).
...
The effect of the target size has been compensated during the postprocessing.
