...
Each UGV only needs to broadcast the sensor measurements to its neighbors via single hopping and then implements individual Bayesian filter locally using its own measurements and the ones transmitted from neighbors.
...
The Latest-In-and-Full-Out (LIFO) protocol is introduced to reduce the communication burden, 
which scales the transmission data linearly with the UGV number, while a statistics dissemination strategy can suffer from the order of the environment size.
...
The proposed LIFO-based DBF has the following properties:
...
(2)	The corresponding DBF ensures the consistency of estimated target position, i.e., the estimate converges in probability to the true value when the number of measurements tends to infinity.
...
Experiment results are presented in \cref{sec:exp}.
...
\section{Indoor Experiment using Mobile Robots}\label{sec:exp}
...
%\end{figure}
We have tested the effectiveness of LIFO-DBF for target localization in simulations and the indoor experiment. 
Due to the space limit, we only include the indoor experiment in the paper and provide the simulation results as the supplemental material.
%conducted a target localization experiment to validate the LIFO-DBF approach in an indoor environment.
The experiment equipments and environment are illustrated in \Cref{fig:exp_scene}.
